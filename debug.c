...
void draw_segment(b2Vec2 p1, b2Vec2 p2, b2HexColor color, [[maybe_unused]] void* context) {
...
void draw_polygon(const b2Vec2* vertices, int vertexCount, b2HexColor color, void* context) {
	for (size_t i = 0; i < (size_t)vertexCount - 1; i++) {
...
void draw_solid_polygon(b2Transform transform, const b2Vec2* vertices, int vertexCount, [[maybe_unused]] float radius,
	b2HexColor color, [[maybe_unused]] void* context) {
    for (size_t i = 0; i < (size_t)vertexCount; ++i) {
        int next_index = (i + 1 == (size_t)vertexCount) ? 0 : i + 1;
...
void draw_point(b2Vec2 p, float size, b2HexColor color, [[maybe_unused]] void* context ) {
